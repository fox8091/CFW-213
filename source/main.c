...
void _start(void){
	*((vu32 *)0x10000020) = 0x340; //Undocumented CONFIG Register, Allows SD/MMC Access
	...
	if(f_open(&handle, "firm.bin", FA_READ | FA_OPEN_EXISTING) == FR_OK){
		...
		u8 sigpattern1[0x8] = {0x4D, 0x22, 0xB5, 0x70, 0x68, 0x69, 0x00, 0x0C};
		u8 sigpattern2[0x4] = {0xE7, 0x76, 0x1C, 0xC0};
		u8 sigpatch1[0x4] = {0x47, 0x70, 0x20, 0x00};
		u8 sigpatch2[0x4] = {0xE7, 0x76, 0x20, 0x00};
